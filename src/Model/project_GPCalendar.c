...
Project_GPCalendar* CreerProject(char* nom, ListMedecin* workingMedecins, ListPatient* consultingPatient, Calendrier calendrier){
    Project_GPCalendar* p = (Project_GPCalendar*) malloc(sizeof(Project_GPCalendar));
 ...
 */
char* Project_jsonSave(Project_GPCalendar* project)
{
...
Project_GPCalendar*  GPCalendar_loadProject(char* nomFichier){
    ...
    if (loadingFile != NULL)
    {
        ...
        Project_GPCalendar* p = Project_jsonLoad(content);
 ...
 */
Project_GPCalendar* Project_jsonLoad(const char* const content){

    Project_GPCalendar* project = NULL;
